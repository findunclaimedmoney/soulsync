...
Deno.serve(async (req) => {
  try {
    ...
    if (order.status === 'paid') return Response.json({ status: 'paid' });
    if (order.status === 'expired') return Response.json({ status: 'expired' });

    if (Date.now() - new Date(order.created_date).getTime() > 24 * 3600000) {
      await base44.entities.CryptoOrder.update(order.id, { status: 'expired' });
